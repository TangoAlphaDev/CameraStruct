...
TSubclassOf<ULyraCameraMode> ULyraHeroComponent::DetermineCameraMode() const
{
	...
	// @TangoAlphaDev
	// Check if the pawn is a LyraCharacter and if it should use the left camera mode
	const ALyraCharacter* LyraCharacter = Cast<ALyraCharacter>(Pawn);
	if (LyraCharacter && LyraCharacter->GetLeftCameraMode())
	{
		if (ULyraPawnExtensionComponent* PawnExtComp = ULyraPawnExtensionComponent::FindPawnExtensionComponent(Pawn))
		{
			if (const ULyraPawnData* PawnData = PawnExtComp->GetPawnData<ULyraPawnData>())
			{
				return PawnData->LeftCameraMode;
			}
		}
	}
	// @TangoAlphaDev
...
void ULyraHeroComponent::SetAbilityCameraMode(TSubclassOf<ULyraCameraMode> CameraMode, const FGameplayAbilitySpecHandle& OwningSpecHandle)
{
	...
	{
		// @TangoAlphaDev
		// Remove any existing entry with the same spec handle
		AbilityCameraModeEntries.RemoveAll([&OwningSpecHandle](const FAbilityCameraModeEntry& Entry) {
			return Entry.SpecHandle == OwningSpecHandle;
		});

		// Add the new entry
		AbilityCameraModeEntries.Add(FAbilityCameraModeEntry(CameraMode, OwningSpecHandle));
...
{
	// @TangoAlphaDev
	// Remove the entry with the given spec handle
	AbilityCameraModeEntries.RemoveAll([&OwningSpecHandle](const FAbilityCameraModeEntry& Entry) {
		return Entry.SpecHandle == OwningSpecHandle;
	});

	// Restore the last camera mode, if any
	if (AbilityCameraModeEntries.Num() > 0)
	{
		const FAbilityCameraModeEntry& LastEntry = AbilityCameraModeEntries.Last();
		AbilityCameraMode = LastEntry.CameraMode;
		AbilityCameraModeOwningSpecHandle = LastEntry.SpecHandle;
	}
	else
	{
		AbilityCameraMode = nullptr;
